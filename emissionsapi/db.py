...
from sqlalchemy import and_, or_, create_engine, Column, DateTime, Integer, \
        Float, String, PickleType, select, insert, func
...
def insert_dataset(session, data, tbl=Carbonmonoxide):
    ...
    values = select([func.unnest(data.value),
                     func.unnest(data.timestamp),
                     func.ST_MakePoint(
                         func.unnest(data.longitude),
                         func.unnest(data.latitude))])
    query = insert(tbl).from_select(columns, values)
